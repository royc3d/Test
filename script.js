...
function updateTable() {
    ...
    expenses.forEach((expense, index) => {
        ...
        row.innerHTML = `
            <td data-label="Opis">${expense.description}</td>
            <td data-label="Znesek (€)">${expense.amount.toFixed(2)} €</td>
            <td data-label="Datum">${expense.date}</td>
            <td data-label="Akcija"><button onclick="deleteExpense(${index})">Izbriši</button></td>
        `;
